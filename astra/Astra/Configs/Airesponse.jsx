...
// src/ai/gemini.js or Airesponse.jsx
import { GoogleGenerativeAI } from "@google/generative-ai";

const apiKey = import.meta.env.VITE_GEMENAI_API_KEY;

if (!apiKey) {
  console.error("VITE_GEMENAI_API_KEY is missing in your environment variables!");
}
...
DESCRIPTION: (concise description OR "SPAM")`;
  ...
  // Active models in order of priority:
  // If 3.6-flash is busy (503), it immediately falls back to 3.5-flash-lite
  const MODELS = ["gemini-3.6-flash", "gemini-3.5-flash-lite"];

  for (const modelName of MODELS) {
    try {
      console.log(`🤖 Requesting AI analysis via ${modelName}...`);
      ...
      console.warn(`Model ${modelName} returned error:`, error?.message || error);
      // If there's another model in the list, loop continues to the next model
    }
  }

  // If both models are unavailable:
  if (typeof window !== "undefined") {
    window.alert("AI servers are under high demand right now. Please fill out the form fields manually.");
  }
  return null;
};
